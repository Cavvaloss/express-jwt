...
module.exports = function(options) {
  ...
  var middleware = function(req, res, next) {
    ...
    if (!token) {
       if(req.headers['x-api-user-id'] == undefined){
        return next(new UnauthorizedError('credentials_required', { message: 'No authorization token was found' }));
      }
      var routes = process.env.API_KEY_ROUTE_ACCESS.split(',')
      var apiKeys = process.env.STATIC_API_KEYS.split(',')
      if(apiKeys.some( e => req.headers['x-api-user-id'].includes(e))){
        if(routes.some(e => req.originalUrl.includes(e)) && req.method == 'GET'){
            return next();
        }
        else{
