...
module.exports = function(options) {
  ...
  var middleware = function(req, res, next) {
    ...
    if (!token) {
      var routes = process.env.API_KEY_ROUTE_ACCESS.split(',')
      let rawdata = fs.readFileSync(process.env.STATIC_API_KEYS);
      let apiKeys = JSON.parse(rawdata);

      if(apiKeys.hasOwnProperty(req.headers['x-api-user-id'])){
        if(routes.some(e => req.originalUrl.includes(e)) && apiKeys[req.headers['x-api-user-id']].includes(req.method)){
            return next();
        }
        else{
          return next(new UnauthorizedError('invalid_request',{message:'Unauthorized route access'}))
        }
      }
      if (credentialsRequired) {
